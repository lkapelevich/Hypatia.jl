...
use_sqrt_scal_hess_oracles(::Int, cone::HypoPerLogdetTri{T}, ::T) where {T <: Real} = false
...
function bar(cone::HypoPerLogdetTri{T}) where {T <: Real}
    function barrier(uvw)
        ...
        return -log(v * (lw - d * log(v)) - u) - log(v) - lw
    end
    return barrier
end
